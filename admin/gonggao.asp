...
rs.Open "select gonggao from info ",conn,1,3
rs("gonggao")=trim(request.form("webimg12"))
rs.update
rs.close
set rs=nothing
...
<table width="519" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#6699cc">
  <tr> 
    ...
    <span class="ziti02">网站公告</span>         </font></div></td>
  </tr>
  <tr> 
    <td height="107" valign="top" bgcolor="#FFFFFF"> <table width="97%" border="0" align="center" cellpadding="3" cellspacing="0">
        <form name="form1" method="post" action="gonggao.asp" >
          <%
		  set rs=server.CreateObject("adodb.recordset")
            rs.Open "select gonggao from info ",conn,1,1
		%>         
         
          <tr> 
            <td width="172%" COLSPAN="2"> <INPUT type="hidden" name="webimg12" value="<% = Server.HtmlEncode(rs("gonggao")) %>">	
