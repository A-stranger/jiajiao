...
strsql = "select * from yufu where intime  between #"&rq&"# and #"&rq2&"# and userid="&id
...
<table width="683" height="108" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#6699cc">
  <tr>
    <td width="681" height="26"><div align="center" class="ziti02">预付费管理</div></td>
  </tr>
  <tr>
    <td height="79" valign="top" bgcolor="#FFFFFF"><br>
	 ...
	 <tr>
	   <td height="19" colspan="2" align="center" bgcolor="#FFFFFF" class="ziti03"><form id="form1" name="form1" method="post" action="yufu.asp">
         <label></label>
         <div align="center">
           <label>请先输入教员编号：
           ...
           注意 ： 
           请不要加上 “T”&nbsp;</div>
	     </form><form action="yufu.asp" method="post" name="form2" class="ziti03" id="form2">
	       按教员姓名查询：
           <label>
	 ...
	 <tr>
       ...
       <% Else %>
       教员编号<span class="STYLE1">T<%= rs("id") %> &nbsp; <%= rs("xingming") %></span>的预付费管理      
       &nbsp; <a href="yufunew.asp?id=<%= id %>" class="top">添加记录</a><br />
       查看该教员: <a href="jiandang.asp?id=<%= id %>" class="top">&nbsp; 认证费</a> <a href="zhongjie.asp?id=<%= id %>" class="top">&nbsp;&nbsp; 中介费</a>
       <% 
	   End If
	   rs.close
	   set rs=nothing
	 ...
	 <tr>
       ...
       <table width="96%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000">
         <tr>
           <td height="33" colspan="3" align="center" class="ziti03">预付费状况</td>
           </tr>
         <tr>
...
set rs=server.CreateObject("adodb.recordset")
strsql="select * from yufu where userid="&id
rs.open strsql,conn,1,1
if rs.eof and rs.bof then 
      %>
      ...
      <tr>
		...
		%>
         <tr>
           ...
           <td align="center" class="ziti03"><%= rs("yufu") %></td>
           <td align="center" class="ziti03"><a href="yufudel.asp?id=<%= rs("id") %>&userid=<%= id %>" class="top">删除</a></td>
         </tr>
         <% 
		 rs.movenext  									'指向下一条记录
		if rs.eof then exit do  						'已经到底最后一条记录则跳出
