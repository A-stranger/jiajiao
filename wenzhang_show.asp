        ...
        <TR>
          <TD width=712 align="left">
            ...
            cellSpacing=0 cellPadding=0 width=712>
              <TBODY>
              <TR>
                <TD height="87" align="center" class="px24"><%= rs("title") %></TD>
              </TR></TBODY></TABLE></TD>
          <TD width=12></TD>
                    ...
                    <table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
                      <tr>
                        <td><%= rs("neirong") %>
						  ...
						  a1=rs("id")
						   rs.close
						set rs=nothing %><BR><BR>
                    <%      
       
	    Response.Write("上一篇:")
		 set rs1=server.CreateObject("adodb.recordset")
         strsql1=" select * from news where type='"&type1&"' and id>"&a1&" "
         rs1.open strsql1,conn,1,1
		 if  rs1.eof  then 
		 Response.Write("没有了&nbsp;&nbsp;")
		 else
		 %>
                    <a href="wenzhang_show.asp?id=<%= rs1("id") %> " class="top1"><%= rs1("title") %></a>&nbsp;&nbsp;
                    <% End If %>
                    <%
         ...
         strsql2="select * from news where type='"&type1&"'and id<"&a1&" order by id desc"
         rs2.open strsql2,conn,1,1
		 if  rs2.eof  then 
		 Response.Write("没有了&nbsp;&nbsp;")
		 else
		 %>
                    <a href="wenzhang_show.asp?id=<%= rs2("id") %>" class="top1"><%= rs2("title") %></a>
                    <%  end if	%></td>
                      </tr>
                    </table>
