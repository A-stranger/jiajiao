...
if upload.form("act")="uploadfile" then
 ...
 if filelx="rar" then
	if fileext<>"rar"  then
		response.write "<script language=javascript>alert('只能上传rar格式的文件，请先打包！');history.go(-1);</script>"
		response.end
	end if
 end if
 if filelx="jpg" then
 ...
 if filelx="rar" then
	if file.filesize>(2000*1024) then
		response.write "<script language=javascript>alert('rar文件大小不能超过2m！');history.go(-1);</script>"
		response.end
	end if
 end if
 if filelx="jpg" then
